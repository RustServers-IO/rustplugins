...
{
    [Info("NTeleportation", "Nogrod", "1.0.5", ResourceId = 1832)]
    class NTeleportation : RustPlugin
    {
        ...
        private void cmdChatTeleportAccept(BasePlayer player, string command, string[] args)
        {
            ...
            TeleportTimers[originPlayer.userID] = new TeleportTimer
            {
                ...
                {
                    err = CheckPlayer(originPlayer, configData.TPR.UsableOutOfBuildingBlocked, CanCraftTPR(originPlayer)) ?? CheckPlayer(player);
                    if (err != null)
                    {
		...
		Dictionary<ulong, Vector3> GetHomesS(string playerS)
		{
			var playerId = Convert.ToUInt64(playerS);
			return GetHomes(playerId);
		}

		Dictionary<ulong, Vector3> GetHomes(ulong playerId)
		{
			if (Home.ContainsKey(playerId))
		        return null;

			if (Home[playerId].Locations.Count == 0)
		    	return null;

			return Home[playerId].Locations;
		}
